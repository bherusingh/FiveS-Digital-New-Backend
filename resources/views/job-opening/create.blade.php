...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('job-opening.store') }}" autocomplete="off" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            @method('post')
            ...
            <div class="card ">
                ...
                <div class="row">
                    ...
                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                      <select class='form-control' name='location' required="true" aria-required="true">
						...
						@foreach($locations as $lid=>$lnm)
							<option value="{{$lid}}" {{ old('location') == $lid ? 'selected' : '' }}>{{$lnm}}</option>
						@endforeach
					  </select>
                  ...
                  <div class="col-sm-2">
                    <div class="form-group{{ $errors->has('min_experience') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('min_experience') ? ' is-invalid' : '' }}" name="min_experience" id="input-min_experience" type="number" placeholder="{{ __('Min') }}" value="{{ old('min_experience') }}" required="true" aria-required="true"/>
				  ...
				  <div class="col-sm-2">
                    <div class="form-group{{ $errors->has('max_experience') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('max_experience') ? ' is-invalid' : '' }}" name="max_experience" id="input-max_experience" type="number" placeholder="{{ __('Max') }}" value="{{ old('max_experience') }}" required="true" aria-required="true"/>
                  ...
                  <div class="col-sm-4">
                    <div class="form-group{{ $errors->has('count') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('count') ? ' is-invalid' : '' }}" name="count" id="input-count" type="number" placeholder="{{ __('Total Post') }}" value="{{ old('count') }}" required="true" aria-required="true"/>
                      @if ($errors->has('count'))
				  ...
				  <div class="col-sm-10">
					<div class="form-group {{ $errors->has('apply_form_link') ? ' has-danger' : '' }}">
						<input class="form-control{{ $errors->has('apply_form_link') ? ' is-invalid' : '' }}" name="apply_form_link" id="input-apply_form_link" type="text" placeholder="{{ __('Apply Form Link') }}" value="{{ old('apply_form_link') }}">
                  ...
                  <div class="col-sm-10">
                    <div class="form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" name="description" id="input-description" type="text" placeholder="{{ __('Short Description') }}" value="{{ old('description') }}" required="true" aria-required="true"/>
                      @if ($errors->has('description'))
                        <span id="description-error" class="error text-danger" for="input-description">{{ $errors->first('description') }}</span>
                      @endif
                    </div>
                  </div>
				  <label class="col-sm-2 col-form-label">{{ __('Job Description Points') }} <a href="#" id="addSummaryPoint" class='btn btn-sm btn-primary'><i class='fa fa-plus'></i> Add More Points</a></label>
				  <div class='col-sm-10'>
					<div class="form-group summary_points-inputs {{ $errors->has('summary_points.*') ? ' has-danger' : '' }}">  	
						<input type="text" class="form-control" name="summary_points[]" placeholder="" value="{{ old('summary_points.0') }}" id='summary_points_1'>
					</div>
              ...
              <div class="card-footer ml-auto mr-auto">
                <button type="submit" class="btn btn-primary">{{ __('Add Job') }}</button>
              </div>
            </div>
          </form>
        </div>
  ...
  <script>
	...
	$('#removeSummaryPoint').on('click',function (e) {
		...
		//$('.summary_points-inputs ').remove();
		//$('.summary_points-inputs ').append("\n"+"<input type='text' class='form-control' name='summary_points[]' placeholder=''>");
	});
</script>
@endsection
