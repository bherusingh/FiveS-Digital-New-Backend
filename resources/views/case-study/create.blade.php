...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('case-study.store') }}" autocomplete="off" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            @method('post')

            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">{{ __('Add Case Study') }}</h4>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('category') ? ' has-danger' : '' }}">
					  <select class="form-control select2 {{ $errors->has('category') ? ' is-invalid' : '' }}" name="category[]" id="input-category" required="true" aria-required="true" multiple="multiple" placeholder="Select Category">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('tag') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('tag') ? ' is-invalid' : '' }}" name="tag" id="input-tag" type="text" placeholder="{{ __('Tag') }}" value="{{ old('tag') }}" required="true" aria-required="true"/>
                      @if ($errors->has('tag'))
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('meta_keyword') ? ' has-danger' : '' }}">
                      <textarea class="form-control{{ $errors->has('meta_keyword') ? ' is-invalid' : '' }}" name="meta_keyword" id="input-meta_keyword" type="text" placeholder="{{ __('Meta Keyword') }}">{{ old('meta_keyword') }}</textarea>
				...
				<div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('meta_description') ? ' has-danger' : '' }}">
                      <textarea class="form-control{{ $errors->has('meta_description') ? ' is-invalid' : '' }}" name="meta_description" id="input-meta_description" placeholder="{{ __('Meta Description') }}">{{ old('meta_description') }}</textarea>
				 ...
				 <div class="row">
                   ...
                   <div class="col-sm-12">
					...
					<textarea id="input-html" name="html" class="form-control{{ $errors->has('html') ? ' is-invalid' : '' }}"  required="true" aria-required="true">{{ old('html') }}</textarea>
                       @if ($errors->has('html'))
              ...
              <div class="card-footer ml-auto mr-auto">
                <button type="submit" class="btn btn-primary">{{ __('Add Case Study') }}</button>
              </div>
            </div>
          </form>
        </div>
