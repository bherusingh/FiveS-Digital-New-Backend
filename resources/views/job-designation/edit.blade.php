...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('job-designation.update', $jobDesignation->id) }}" autocomplete="off" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            @method('put')
            ...
            <div class="card ">
              ...
              <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                      <select class='form-control' name='department' required="true" aria-required="true">
						...
						@foreach($departments as $dpid=>$dpnm)
							<option value="{{$dpid}}" @if(old('department',$jobDesignation->department) == $dpid) selected @endif>{{$dpnm}}</option>
						@endforeach
                  ...
                  <div class="col-sm-4">
                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-name" type="text" placeholder="{{ __('designation') }}" value="{{ old('name',$jobDesignation->name) }}" required="true" aria-required="true"/>
                      @if ($errors->has('name'))
