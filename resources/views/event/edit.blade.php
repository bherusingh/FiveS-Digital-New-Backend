...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('event.update', $event->id) }}" autocomplete="off" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            @method('put')
                ...
                <div class="row">
                  ...
                  <div class="col-sm-4">
                    <div class="form-group{{ $errors->has('is_active') ? ' has-danger' : '' }}">
					  <select class='form-control' name='is_active' required="true" aria-required="true">
						...
						<option value='0' {{ old('is_active',$event->is_active) == 0 ? 'selected' : '' }}>Deactive</option>
					  </select>	
				  ...
				  <div class="col-sm-4">
					<div class="form-group {{ $errors->has('start_date') ? ' has-danger' : '' }}">
						<input class="form-control{{ $errors->has('start_date') ? ' is-invalid' : '' }}" name="start_date" id="input-start_date" type="datetime-local" placeholder="{{ __('Start Date') }}" 
						value="{{ old('start_date',date('Y-m-d\TH:i',strtotime($event->start_date))) }}">
				  ...
				  <div class="col-sm-4">
					<div class="form-group {{ $errors->has('end_date') ? ' has-danger' : '' }}">
						<input class="form-control{{ $errors->has('end_date') ? ' is-invalid' : '' }}" name="end_date" id="input-end_date" type="datetime-local" placeholder="{{ __('End Date') }}" 
						value="{{ old('end_date',date('Y-m-d\TH:i',strtotime($event->end_date))) }}">
				  ...
				  <div class="col-sm-4">
					   <div class="fileinput fileinput-new text-center" data-provides="fileinput">
							<div class="fileinput-preview fileinput-exists thumbnail img-raised">
								@if($event->img_path)
									<img src="{{ env('FRONT_URL').'/front/public/assets/images/'.$event->img_path }}" style='max-width:95%;'/>
								@endif
							</div>
								<div>
				  ...
				  <input type='hidden' value="{{ old('old_img_path',$event->img_path) }}" name="old_img_path">
				</div>
				<div class="row">
				  <label class="col-sm-2 col-form-label">{{ __('Description') }}</label>
                  <div class="col-sm-10">
                    <div class="form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                      <textarea rows="5" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" name="description" id="input-description" placeholder="{{ __('Description') }} " required="true" aria-required="true">{{ old('description',$event->description) }}</textarea>
